...
from kantts.models.sambert.kantts_sambert import KanTtsSAMBERT, KanTtsTextsyBERT, KanTtsSAMBERTHalfOV, KanTtsSAMBERTHalfWithoutOV  # NOQA
...
def hifigan_model_builder(is_ov_convert, config, device, rank, distributed):
    model = {}
    optimizer = {}
    scheduler = {}
    model["discriminator"] = {}
...
#  TODO: some parsing
def sambert_model_builder(is_ov_convert, config, device, rank, distributed):
    model = {}
    optimizer = {}
    scheduler = {}

    if is_ov_convert:
        model["KanTtsSAMBERT"] = KanTtsSAMBERTHalfOV(
            config["Model"]["KanTtsSAMBERT"]["params"]
        ).to(device)
    else:
        model["KanTtsSAMBERT"] = KanTtsSAMBERTHalfWithoutOV(
            config["Model"]["KanTtsSAMBERT"]["params"]
        ).to(device)
...
def sybert_model_builder(is_ov_convert, config, device, rank, distributed):
    model = {}
    optimizer = {}
    scheduler = {}
...
def model_builder(is_ov_convert, config, device="cpu", rank=0, distributed=False):
    builder_func = model_dict[config["model_type"]]
    model, optimizer, scheduler = builder_func(is_ov_convert, config, device, rank, distributed)
    return model, optimizer, scheduler
