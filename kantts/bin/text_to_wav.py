...
def concat_process(chunked_dir, output_dir):
    ...
    while cnt < len(wav_files):
        wav_file = os.path.join(
            chunked_dir, "{}_{}_mel_gen_ov.wav".format(main_id, sub_id)
        )
...
def text_to_wav(
    ...
    os.makedirs(os.path.join(output_dir, "res_wavs_ov"), exist_ok=True)
    ...
    if is_ov_convert:
        logging.info("AM is converting...")
    else:
        logging.info("AM is infering...")
    am_infer(is_ov_convert, symbols_file, am_ckpt, output_dir, se_file)
    print("-------------Sambert hybrid pipeline for 30 short sentences cost time: %f s-------------"%(time.time() - start))
    if is_ov_convert:
        logging.info("Vocoder is converting...")
    else:
        logging.info("Vocoder is infering...")
    start1 = time.time()
    hifigan_infer(is_ov_convert,os.path.join(output_dir, "feat"), voc_ckpt, output_dir)
    print("-------------Hifigan ov pipeline for 30 short sentences cost time: %f s-------------"%(time.time() - start1))

    concat_process(output_dir, os.path.join(output_dir, "res_wavs_ov"))

    print("-------------TTS hybrid pipeline for 30 short sentences cost time: %f s------------- "%(time.time() - start))
    logging.info("Text to wav finished!")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Text to wav")
    parser.add_argument("--is_ov_convert", action="store_true", help="is convert to ov")
