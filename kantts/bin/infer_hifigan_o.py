...
def hifigan_infer(input_mel, ckpt_path, output_dir, config=None):
    ...
    # core = ov.Core()
    # ov_model = core.read_model('/home/chentianmeng/workspace/TTS/KAN-TTS/hifigan_t.xml')
    ...
    # print("-------------Hifigan ov model infer for 5 long sentences costs time: %f ms-------------"%(time.time()* 1000 - start1))
    print("-------------Hifigan torch model infer for 30 short sentences costs time: %f ms-------------"%(time.time()* 1000 - start1))
    # report average RTF
    logging.info(
